import { Suspense } from 'react';
import { Edit, Plus } from 'lucide-react';
import { createClient } from '@/lib/supabase/server';
import { DeleteBannerButton } from '@/components/admin/DeleteBannerButton';
import { ProductTableSkeleton } from '@/components/admin/ProductTableSkeleton';

async function BannersList() {
  const supabase = await createClient();

  let banners: any[] = [];
  let error: string | null = null;

  try {
    const { data, error: fetchError } = await supabase
      .from('banners')
      .select('id, title, placement, start_date, end_date, is_active')
      .order('created_at', { ascending: false });

    if (fetchError) throw fetchError;
    banners = data || [];
  } catch (err) {
    error = 'Gagal mengambil data banner';
    console.error(err);
  }

  if (error) {
    return (
      <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg">
        {error}
      </div>
    );
  }

  if (banners.length === 0) {
    return (
      <div className="bg-gray-50 rounded-lg p-8 text-center">
        <p className="text-gray-600 mb-4">Belum ada banner.</p>
        <a
          href="/admin/banners/new"
          className="inline-flex items-center gap-2 bg-primary text-white font-bold py-2 px-6 rounded-lg hover:opacity-90 transition-opacity"
        >
          <Plus size={20} />
          Tambah Banner Pertama
        </a>
      </div>
    );
  }

  const placementLabels: { [key: string]: string } = {
    promo_bar: 'Promo Bar',
    hero: 'Hero',
    promo_section: 'Promo Section',
  };

  const formatDate = (dateStr: string | null) => {
    if (!dateStr) return '-';
    return new Date(dateStr).toLocaleDateString('id-ID', {
      year: 'numeric',
      month: 'short',
      day: 'numeric',
      hour: '2-digit',
      minute: '2-digit',
    });
  };

  return (
    <div className="bg-white rounded-lg overflow-hidden shadow">
      <table className="w-full">
        <thead className="bg-gray-50 border-b border-gray-200">
          <tr>
            <th className="px-6 py-3 text-left text-sm font-bold text-foreground">Judul</th>
            <th className="px-6 py-3 text-left text-sm font-bold text-foreground">Penempatan</th>
            <th className="px-6 py-3 text-left text-sm font-bold text-foreground">Mulai</th>
            <th className="px-6 py-3 text-left text-sm font-bold text-foreground">Berakhir</th>
            <th className="px-6 py-3 text-left text-sm font-bold text-foreground">Status</th>
            <th className="px-6 py-3 text-right text-sm font-bold text-foreground">Aksi</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-200">
          {banners.map((banner) => (
            <tr key={banner.id} className="hover:bg-gray-50 transition-colors">
              <td className="px-6 py-4 text-sm text-foreground font-medium">{banner.title}</td>
              <td className="px-6 py-4 text-sm text-gray-600">
                {placementLabels[banner.placement] || banner.placement}
              </td>
              <td className="px-6 py-4 text-sm text-gray-600">{formatDate(banner.start_date)}</td>
              <td className="px-6 py-4 text-sm text-gray-600">{formatDate(banner.end_date)}</td>
              <td className="px-6 py-4 text-sm">
                <span
                  className={`px-3 py-1 rounded-full text-xs font-bold ${
                    banner.is_active
                      ? 'bg-green-100 text-green-700'
                      : 'bg-gray-100 text-gray-700'
                  }`}
                >
                  {banner.is_active ? 'Aktif' : 'Tidak Aktif'}
                </span>
              </td>
              <td className="px-6 py-4 text-right text-sm">
                <div className="flex items-center justify-end gap-2">
                  <a
                    href={`/admin/banners/${banner.id}/edit`}
                    className="inline-flex items-center gap-1 text-primary hover:opacity-70 transition-opacity"
                  >
                    <Edit size={18} />
                  </a>
                  <DeleteBannerButton bannerId={banner.id} bannerTitle={banner.title} />
                </div>
              </td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  );
}

export default function BannersPage() {
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold font-heading text-foreground">Kelola Banner</h1>
          <p className="text-gray-600 mt-2">Atur banner dan gambar promosi</p>
        </div>
        <a
          href="/admin/banners/new"
          className="inline-flex items-center gap-2 bg-primary text-white font-bold py-2 px-6 rounded-lg hover:opacity-90 transition-opacity"
        >
          <Plus size={20} />
          Tambah Banner
        </a>
      </div>

      <Suspense fallback={<ProductTableSkeleton />}>
        <BannersList />
      </Suspense>
    </div>
  );
}
